...
# ==================== CONFIGURATION ====================
BOT_TOKEN = '8848196143:AAGfhBSsiaL-30TRMQ11tpE8rRKnBl7rB10'
...
╚══════════════════════════════════╝"""
    ...
    for btn_text, cfg in lookups.items():
        if text == btn_text:
            ...
            # NOTE: format_func NOT saved to state (functions can't be JSON serialized)
            if 'pattern' in cfg:
    ...
    # Map stage -> format function (since functions can't be stored in JSON state)
    stage_format_map = {
        'awaiting_number': format_number_response,
        'awaiting_aadhaar': format_aadhaar_response,
        'awaiting_family': format_family_response,
        'awaiting_pincode': format_pincode_response,
        'awaiting_ifsc': format_ifsc_response,
        'awaiting_instagram': format_instagram_response,
        'awaiting_telegram': format_telegram_response,
        'awaiting_vehicle': format_vehicle_response,
    }

    # Handle input for lookups
    if state.get('stage') in stage_format_map:
        query = text
        ...
        if query:
            ...
            format_func = stage_format_map[state['stage']]
